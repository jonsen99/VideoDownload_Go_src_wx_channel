...
#### 设备能力同步
- ✅ Hub 现在会接收客户端上报的页面能力摘要
- ✅ 持久化记录设备当前页面、当前 URL、API 就绪状态
- ✅ 记录搜索 / 视频列表 / 视频详情能力以及 ready 页面数量

#### 远程调用选择优化
- ✅ `/api/call` 自动选择设备时，不再仅按“第一个在线设备”处理
- ✅ 搜索请求优先选择支持搜索的设备
- ✅ 视频列表请求优先选择支持 feed_list 的设备
- ✅ 视频详情请求优先选择支持 feed_profile 的设备
- ✅ 当没有 ready 设备时，返回更明确的错误提示

#### 前端状态页增强
- ✅ Dashboard 增加“搜索就绪 / 列表就绪 / 详情就绪”统计卡片
- ✅ 设备卡片显示当前页面路径与能力标签
- ✅ Search 页面显示当前设备 API 就绪状态
- ✅ “自动选择 / 切换到最佳设备”优先选择真正可搜索的设备

### 技术说明

- 客户端通过心跳同步页面能力到 Hub
- Node 模型新增页面能力相关字段
- `/api/clients` 现在返回解析后的方法集合与能力状态
